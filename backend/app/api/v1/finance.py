...
@router.get("/cash-sessions/current/", response_model=CashSessionRead)
def get_current_session(
    ...
    if not session:
        raise HTTPException(status_code=404, detail="No open cash session found for this user")
    return session
...
def close_cash_session(
    ...
    # Requirement: Force closing_notes if difference > 5 USD (combined value or just USD?)
    # Let's check combined or just USD for simplicity as per original
    if (abs(diff_usd) > 5 or abs(diff_ves / (rate.rate if rate else 1)) > 5) and not session_in.notes:
        raise HTTPException(
            status_code=400, 
    ...
    transaction = CashTransaction(
        ...
        exchange_rate=rate.rate if rate else 0,
